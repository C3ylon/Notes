...
`a, b = c, d`操作的原理是先暂存元组`(c, d)`，然后解包暂存的元组，将值从左至右依次赋值给`a`和`b`。因此为了互换`a[i]`和`a[a[i]]`两处的值若使用`a[i], a[a[i]] = a[a[i]], a[i]`则`a[i]`会先被赋值，导致 `a[a[i]]`指向的元素出错。正确的写法是`a[a[i]], a[i] = a[i], a[a[i]]`。
