...
在定义变量时使用等号的都是复制初始化，未使用等号的都是直接初始化。
...
`T var = val;`与`T var = { val }`（即复制初始化和复制列表初始化）都不会调用`explicit`修饰的构造函数和自定义转换函数。
...
> 在C++17之前，执行`T var = val;`形式的复制初始化时，如果`val`的类型不是`T`类型，会先把`val`的类型通过转换构造函数(*converting constructor*)隐式转换为`T`类型，然后再将该临时量以**直接初始化**的形式来初始化`var`。
>
