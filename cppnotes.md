...
如果是常左值引用(`const T &var = val;`)或右值引用(`T &&var = val;`)，当无法获取到所用初始化对象的地址时，会"申请"一个临时内存空间，然后以**拷贝初始化**的形式初始化这片内存空间，再将临时内存空间的地址存入引用变量的内存空间。临时内存空间的生命周期与引用变量的生命周期相同。
...
struct st3 { st3(st2) { } };
// const st3 &a = st1{};
// 错误，临时内存空间的初始化方式是拷贝初始化
// 即相当于st3 tmp = st1{};
// st3类型的变量无法由st1类型的初始化对象经非列表形式的拷贝初始化得到
const st3 &a = { st1{} };
```
...
>
> 对于任何显式`inline`和隐式`inline`的函数应当默认其具有外部链接属性，并避免其在不同翻译单元中定义不同。
...
模板函数也可以使用`static`关键字使其具有内部链接属性，这样即使在不同翻译单元中模板函数的定义内容不一样也不会产生非良构。(在部分早期编译器中可能仍会出现类似非良构的表现，因此尽量避免这样使用)
