...
比如类cl的构造函数分别接受A，B，C三种类型，类型C可由类型D隐式转换得到。

在直接初始化cl时，会依次比较能否按照类型A，B，C调用构造函数。如果传参是类型D，则由于隐式转换，会调用接受类型C的构造函数。相当于构造函数类型候选集变大。

在拷贝初始化cl时，构造函数的类型候选集就限定在A，B，C三种类型中，如果传参类型不符合A，B，C三种类型则无法调用构造函数。

若此时用`explicit`修饰接受A类型的构造函数，则在拷贝初始化时会将类型A从构造函数类型候选集中移除，而在直接初始化时由于仍会依次比较每个构造函数的参数类型，因此不受影响。
