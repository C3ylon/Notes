...
对于无符号类型变量，如果所赋值超出其表示范围:

+ 值为正数时：变量的值为所赋值对该无符号类型所能表示数值总数(2^N)取模后的余数。
+ 值为负数时：变量的值为所赋值按 Python 的形式对 2^N 取模的结果。
...
无符号整数类型的等级等于其对应的有符号整数类型的等级。

当值`val`的类型`T`的整数转换等级低于`int`的整数转换等级时，在`val`参与基础计算时：

+ `int`可以表示`T`类型能取到的所有值时，`val`会被整型提升为`int`类型
+ `int`不可以表示`T`类型能取到的所有值时，`val`会被整型提升为`unsigned int`类型

***

无符号整数和有符号整数混合运算时 (逐条判断每条规则)：

+ 当无符号整数转换等级高于或等于有符号整数转换等级时，把有符号整数转换为该无符号整数类型
+ 当有符号整数类型可以表示无符号整数类型能取到的所有值时，把无符号整数转换为该有符号整数类型
+ 将无符号整数和有符号整数都转换为有符号整数类型对应的无符号类型

> 也可以理解为：
>
> + 当有符号类型能完全覆盖无符号类型的取值范围时，把无符号类型转换为该有符号类型计算（注意这种场景下不能发生溢出）
> + 其余情况选择二者之间具有更高整数转换等级的类型的无符号版本作为无符号类型和有符号类型需要转换到的公共类型，转换为公共类型之后再进行计算。

```C++
unsigned int a = 0xffffffff;
int b = 1;
auto c = a + b; // c 类型为 unsigned int，值为 0
long d = 1;
auto e = a + d;
...
1. 整型字面值：
    ...
    + **十进制**字面值对应 `int`, `long`, `long long`中可容纳值的最小宽度类型(不包含无符号版本)。
    + **八进制**和**十六进制**字面值对应`int`, `unsigned int`, `long`, `unsigned long`, `long long`, `unsigned long long`中可容纳值的最小宽度类型。
...
> 背景：C++受从C语言继承的语法解析规则的影响，只要一段代码能够被解析为函数声明，那么编译器就必须最优先将其解析为函数声明。
...
// 如果指定默认值时可以使用小括号初始化的方式
// 这里实际上也只是声明了一个参数类型为 st1() 返回值类型为 st1 的函数
// 为避免这种歧义，C++11标准在引入类内初始化的特性时
// 直接不允许使用小括号初始化的方式
// 并引入了列表初始化这种一定不会被解析为函数声明的初始化方式
```
...
C++标准中对一些场景做出限制实际上是为了收束编译期需要执行的底层逻辑。比如以下例子：

+ 即使复制初始化函数用`explicit`关键字修饰也不影响`T var = val;` 形式的复制初始化（前提是`val`的类型不是`T`类型）。

  在`val`经过类类型转换得到`T`类型的一个临时变量后，该临时变量会对`val`进行直接初始化而不是复制初始化。这是因为如果此时还要复制初始化的话，编译器又会进入到复制初始化的处理流程，所以标准直接规定用类类型转换后的临时变量来直接初始化目标对象，打破编译器执行流程中的"递归"。
  ...
  int main() {
      fn(st1{});
      // fn(st2{});
      // error: no matching constructor for initialization of 'st2'
      return 0;
  }
  ```

  这样看上去好像即使用`explicit`关键字修饰复制构造函数也未能阻止复制初始化。

+ `T var = val;` 形式的复制初始化在遍历`T`构造函数列表时，如果一个构造函数的参数类型需要`val`的类型经过用户定义转换（类类型转换）才能得到，那么不考虑该构造函数。

  标准选择做出这样的限制是为了避免复制初始化时深入递归查找可匹配的参数转换范围。
  ...
  试想一下，当复制初始化允许类类型转换时，写下`st4 a = st1{};`,这样会一直深入递归匹配到 `st2(const st1 &)`构造函数。

  对于直接初始化/直接列表初始化来说，初始化的过程相当于把`T`的整个构造函数列表按重载决议的规则判定调用优先级顺序并调用最佳构造函数，因此传入的实参在初始化其所匹配的构造函数的形参时，一定会"收束"成为复制初始化。

  比如写下`st3 a{ st1{} };`时，在初始化`st3(const st2 &)`构造函数的形参时，等效于`const st2 &_param = st1{}`，这样实际上还是回归到了复制初始化。

  因此为了使初始化时跨越更多的类型转换层级，可以以列表初始化的形式手动多加几层花括号。比如写下`st4 a = { { st1{} } };`时，等效于`const st3 &_param = { st1{} };`，`const st2 &_param = st1{}`。

+ 函数模板只有完全特化，没有偏特化。

  这样设计是因为函数重载已经完全承载了偏特化的功能。如果再引入函数模板偏特化的机制，会对函数重载决议规则产生非常复杂的影响。因此C++标准将函数模板的特化彻底限制在了完全特化。实际上，对于最佳实践来说，函数模板的完全特化也不推荐使用，应当考虑用函数重载来替代特化。
...
一个翻译单元中，某个对象的声明/定义首次出现时就已经确定其链接属性。

+ 若对象的链接属性已经确定为`internal`，则再用`extern`声明不会再更改其链接属性。
+ 若对象的链接属性已经确定为`external`，则再用`static`声明会导致编译错误。
...
从实参到形参的匹配程度由高至低分为以下三个等级:

1. 精确匹配 (**Exact match**)

    在该层级中，对于指针或引用类型的形参来说，如果其底层类型对应的实参没有`const`限定符，则没有底层`const`的形参比有底层`const`的形参匹配优先级更高。

    > + 如果实参类型为`T`，则形参类型`T`和形参类型`const T&`的匹配优先级一样高。因为底层`const`带来的优先级区分仅是针对两个指针或两个引用类型之间而言。（如`T *`和`const T *`，以及`T &`和`const T&`）
    >
...
2. 提升 (**Promotion**)
    ...
    提升包括整型提升和浮点提升。从广义上来说，**提升**也属于**转换**中的一种场景。

    > 注意从`short`到`int`是提升，但从`int`到`long`是转换。
    >
    > 比如重载函数有两个，形参类型别是`int`和`short`，实参类型是`char`，由于`char`到`int`是提升，而`char`到`short`是转换，因此会匹配形参类型为`int`的函数。此外`bool`整型提升之后也是`int`。

3. 转换 (**Conversion**)

    转换分为**算数类型转换**和**类类型转换**两种。（也即**内置类型转换**和**用户定义转换**）

    其中算数类型转换的优先级比类类型转换的优先级更高。

    所有算术类型转换的级别都一样，比如`int`向`unsigned int`的转换并不比`int`向`double`的转换优先级高。

    由算术类型向类类型、由类类型向算术类型、由类类型向类类型都属于类类型转换。
...
此即 **SFINAE** 原则。（**Substitution Failure Is Not An Error**）
...
// destruct 3           -> 析构 1st 异常对象
// copy init 5          -> 复制初始化 2nd catch 对象
// destruct 5           -> 析构 2nd catch 对象
// destruct 4           -> 析构 2nd 异常对象
// copy init 6          -> 复制初始化 3rd 异常对象
// copy init 7          -> 复制初始化 3rd catch 对象
// destruct 7           -> 析构 3rd catch 对象
// destruct 6           -> 析构 3rd 异常对象
// destruct 1           -> 析构a
```
...
// destruct 0-----------执行到throw处类比于函数执行到return处，在将返回值赋予临时量后会析构该函数内的所有局部变量
// copy 2
// fn2 3
// destruct 2-----------若catch语块末有throw; 则不会析构最开始的异常参数
// copy 3
// fn1 3
// destruct 3
// destruct 1-----------若catch语块末没有throw; 则编译器会在此生成最初传入的异常参数的析构函数
```
...
> public:
>     cl() = default;
>     ~cl() noexcept(false) { throw "destruct false"; }
> };
>
> int main()
> {
...
> // 输出 in exception: destruct false
...
+ 模板形参包：`typename... Args`，其中`Args`可用于包展开
+ 函数形参包：`Args &&... args` | `const Args &... args`，其中`args`可用于包展开

> 注意在说明函数形参包时最好加上形参包的名字。因为`...`会与C风格的可变参数列表产生歧义。
>
> 如下代码，第一个`...`会被直接解析为可变参数列表，第二个`...`在gcc中被解析为包展开，而在clang中直接报错 *error: '...' must be innermost component of anonymous pack declaration.*
>
> 如果本意是让第一个`...`解析为函数形参包，则应显式加上形参包的名字，如`void fn(Ret(*(...f)())...)`，这样第二个`...`就会被解析为可变参数列表。
>
> ```C++
...
对于具有外部链接属性的内联函数，如果使用了本翻译单元内定义的具有内部链接属性的静态变量，那么程序**非良构**。

> 对于C++而言，无链接属性的静态变量 (即局部静态变量) 定义在具有外部链接属性的内联函数中时，所有翻译单元之间共享某一个翻译单元之中定义的该静态变量。
>
> 对于C而言，具有外部链接属性的内联函数不能定义非常量局部静态变量。
...
st3 c (st4{});         // 正确
st3 d { st4{} };
// 正确，st3不是POD类型，会考虑到类类型转换后再调用复制构造函数
```
...
C++中的隐式移动(Implicit Move)：在引入移动语义后，对于函数返回的临时量的初始化，会**优先采用移动构造函数**，在移动构造函数不可访问时才会选择复制构造函数。不要显式使用`return std::move(expr);`，这样反而会阻碍编译器的RVO优化。（主要影响第一个场景）
...
2. 指针所指对象始终不违背严格别名规则的三个例外类型：
   ...
   >    }
   >    float e = *(float*)d;          // 违背严格别名，例外类型不能反向兼容
   >    ```
   >
   > 2. 在C中的三个例外类型为：
   >
...
+ 无作用域枚举的基础类型不固定，其基础类型是可以表示所有枚举项的值的整数类型。
  ...
  > 无作用域枚举类型可转换到以下列表中**首个**可以表示所有枚举项的值的类型：
  >
  > + `int`
  > + `unsigned int`
  > + `long`
...
在逗号表达式`E1, E2`中，会先对表达式`E1`求值，然后结果被丢弃（结果如果是类类型，则在完整的表达式结束之后才会析构）。接着对表达式`E2`求值，结果作为整个逗号表达式`E1, E2`的值。

> + 表达式`E1`和表达式`E2`值的类型可以不一致。
> + 如果是重载过的逗号运算符( `operator,()` )，则不能保证`E1`表达式的副作用在`E2`表达式求值开始前完成。因此非常不推荐重载逗号运算符。
...
struct st {
    st() { cout << "init: " << this << endl; }
    ~st() { cout << "destruct: " << this << endl; }
};

void fn(const std::string &) {
    cout << "exp is a string" << endl;
}
...
int main() {
    ...
    // init: 0x16b5aedd3
    // exp is a string
    // destruct: 0x16b5aedd3
    // destruct: 0x16b5aedfb
    return 0;
}
```
...
如果某个静态变量`A`的析构函数中触发了另一个静态变量`B`的构造，那么此时记录析构函数的栈已经将正在执行的析构函数`~A()`弹出（即先出栈后执行），在`B`构造结束时新压入待执行的析构函数`~B()`。对栈中析构函数的处理进行到下一个循环时，会执行析构函数`~B()`。从宏观上来看执行流程就是 `A`构造完成 -> `A`开始析构 -> `B`构造完成 -> `A`析构完成 -> `B`开始析构 -> `B`析构完成，并不是严格意义上的先构造完成的一定会后析构完成。
...
使用局部静态变量时可能存在性能损耗的地方：如果变量的初始值不是编译期常量，那么以后每次调用到该函数时都会触发一次判断，判断该局部静态变量有没有被初始化过。当变量的初始值是编译期常量时，会优化为编译时直接在对应的地址填入初始值，每次调用到该函数时不进行初始化检查。
...
| 内存序 | 指令重排 | 同步性 |
| :--- | :--- | :--- |
| `memory_order_relaxed` | 不禁止指令重排 | 不具备同步性 |
| `memory_order_consume` | *deprecated in C++26* | *deprecated in C++26* |
| `memory_order_acquire` | 前面的读/写指令**可以**被推后，后面的读/写指令**不能**被提前 | 当`acquire`观测到另一个线程通过`release`及以上等级内存序写入的值时，必定能观测到那个线程对应的原子写入操作之前的所有写入操作写入的值 |
| `memory_order_release` | 前面的读/写指令**不能**被推后，后面的读/写指令**可以**被提前 | 当一个线程通过`release`写入值后，如果被其他线程通过`acquire`观测到写入的值，那么必定能被观测到这个原子写入操作之前的所有写入操作写入的值 |
| `memory_order_acq_rel` | 前面的读/写指令**不能**被推后，后面的读/写指令**不能**被提前，仅用于读改写(RMW)操作 | `acquire`和`release`的结合 |
