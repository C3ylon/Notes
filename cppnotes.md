...
+ `T var = val;` 形式的复制初始化在遍历`T`构造函数列表时，如果一个构造函数的参数类型需要`val`的类型经过用户定义转换（类类型转换）才能得到，那么不考虑该构造函数。
  ...
  比如写下`st3 a{ st1{} };`时，在初始化`st3(const st2 &)`构造函数的形参时，等效于`const st2 &_param = st1{}`，这样实际上还是回归到了复制初始化。因此为了使直接初始化/直接列表初始化跨越更多的类型层级，可以手动多加几层小括号/花括号。比如写下`st4 a = { { st1{} } };`时，等效于`const st3 &_param = { st1{} };`，`const st2 &_param = st1{}`。
