...
`.gitignore`文件放在 git 仓库的根目录下即可指定需要忽略（或不忽略）的文件。

注释以`#`开始，且注释必须单起一行。除注释以外的非空白行，每一行指定一种忽略（或不忽略）的范围，不可以在同一行内指定多条。
...
+ `?`：匹配一个字符
+ `[]`：匹配括号内的任一字符。
  > `[abc].cpp` (只匹配 `a.cpp`, `b.cpp`, `c.cpp`)

`!`为指定不忽略文件的符号。后面的设置会覆盖前面的设置。

> 注意当某个文件夹被忽略时，其下任意内容都将会被忽略，此时指定不忽略其中某些内容的设置将会失效。
>
> 比如，如果先设置了`build/`，然后设置`!build/bin/`，并不能取消`build/bin/`文件夹的忽略状态，因为在遍历到`build/`文件夹时就整个忽略了。正确的写法是先设置`build/*`，再设置`!build/bin/`。

外层文件夹的 gitignore 设置会带入到内层文件夹。如果内层文件夹单独配置有`.gitignore`文件，那么其设置等同于加在了外层传入的 gitignore 设置之后，并且后面的设置也能够覆盖前面的设置。

```gitignore
# 递归到最内层忽略所有名称为 build 的文件和文件夹
build
# 递归到最内层忽略所有名称为 build 的文件夹
build/
# 忽略 git 根目录下的名称为 build 的文件和文件夹
/build
# 忽略 git 根目录下的名称为 build 的文件夹
/build/
# 忽略 git 根目录下的名称为 build 的文件夹下的任意内容
build/*
# 递归到最内层忽略所有名称为 build 的文件夹下的任意内容
**/build/*
#
```
