...
>
> 在VS编译器中，使用Unicode字符集和使用多字节字符集的直接区别是：编译器是否增加了宏定义`UNICODE`。而是否增加该宏定义则决定了Windows API函数参数有字符串时使用的多字节字符集还是宽字符字符集。
...
在声明结构体时如遇到需要在该结构体中声明struct或union的情况: 
...
编译器即使把一个函数优化为内联的一段汇编代码，也会在另外一个位置生成调用该函数时需要执行的汇编代码，使得另一个文件中的代码调用该函数时能够找到位置。
