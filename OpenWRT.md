...
以上两种网络结构各有优缺点，下面将分别讲解安装步骤。写这篇笔记时所有操作步骤都是经过一步步复现的。附录部分是一些杂项内容的汇总，主要用于个人记录。
...
> 注意：OpenWrt从`22.03.0`版本开始，将配置防火墙的工具从`iptables`替换为了`nftables`。这两者的配置语法**不兼容**，会导致部分没有及时升级的插件无法运行在较新版本的OpenWrt系统上，比如网易的UU加速器插件，目前不支持`nftables`。如果有使用这类插件的需求，应当选择`21.02.7`及其之前的版本。
...
>
> + `ext4`格式可以扩展磁盘空间大小，而`squashfs`格式不能。
> + `squashfs`格式可以使用重置功能恢复出厂设置，而`ext4`格式不能。
>
> 鉴于本文安装OpenWrt是在虚拟机环境下，可以很方便使用虚拟机快照功能来恢复设置，因此采用`ext4`格式。如果是安装在实体路由器硬件上推荐使用`squashfs`格式。
...
该网络结构只需要用到一个VMnet子网。这里将VMnet2设置为桥接模式，桥接网卡选项由默认的自动改为物理有线网卡。
...
如上图，在右侧的网络连接部分选择自定义，并选择之前设置为桥接模式的VMnet子网（本文是VMnet2）。
...
旁路网关结构使用OpenWrt核心是要让设备的网关和DNS服务器地址指向OpenWrt。为达到这一目的可以有两种方法，其一是在设备自身修改网关和DNS服务器地址，如果设备无法设定自身网关（比如电视盒子），就需要用到另一种方法，即修改主路由DHCP服务器下发的网关和DNS服务器地址。
...
该结构是一个符合规范的**对称网络结构**。推荐尽量使用该结构。唯一的缺点是需要至少双网卡。不过本文是在虚拟机环境下实现，VMware可以添加十余个虚拟网卡，因此这个缺点也就可以忽视。相较于仅使用lan的旁路网关结构来说，使用lan-wan结构的路由有很多好处，最明显的就是在无线网络环境下，宿主机也能使用虚拟机里的OpenWrt来正常上网了。因此下文采用宿主机无线网卡作为给虚拟机OpenWrt提供网络的物理设备。
...
以**管理员模式**运行VMware。创建新虚拟机的步骤与前述步骤相同，到了 *选择磁盘* 界面的时候，选择第三项：使用物理磁盘。选择物理磁盘设备时，再次使用`diskpart`->`list disk`，依据磁盘容量确定U盘的 PhysicalDrive 编号。下面的 *使用情况* 选项选择使用整个磁盘。
...
接下来进入lan区域的防火墙设置，并取消勾选wan区域的转发权限。

![防火墙设置1](./pics/OpenWrt/4.14.png)

在lan区域取消从wan至lan的转发权限后，wan区域会自动取消从wan至lan的转发权限。设置完后如下图所示。
